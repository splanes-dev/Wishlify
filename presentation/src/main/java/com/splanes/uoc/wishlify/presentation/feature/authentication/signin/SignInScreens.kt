...
fun SignInFormScreen(
  ...
  onDismissError: () -> Unit,
  onClearInputError: (input: SignInForm.Input) -> Unit,
  onSignIn: (form: SignInForm) -> Unit,
  onGoogleSignIn: () -> Unit,
  onSignUp: () -> Unit,
) {
  ...
  val emailState = rememberTextInputState(
    onClearError = { onClearInputError(SignInForm.Input.Email) }
  )
  val passwordState = rememberTextInputState(
    onClearError = { onClearInputError(SignInForm.Input.Password) }
  )
...
fun AutoSignInScreen() {
  Box(
    ...
    Loader(
      modifier = Modifier.fillMaxSize(),
      containerColor = WishlifyTheme.colorScheme.background
    )
  }
}